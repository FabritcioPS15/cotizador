...
function Button({ children, primary = false, onClick, className = '' }: { children: React.ReactNode; primary?: boolean; onClick?: () => void; className?: string }) { return <button className={`app-button ${primary ? 'app-button-primary' : ''} ${className}`} onClick={onClick}>{children}</button> }
...
function Select({ children, value, onChange }: { children: React.ReactNode; value?: string; onChange?: (e: React.ChangeEvent<HTMLSelectElement>) => void }) { return <select value={value} onChange={onChange}>{children}</select> }
