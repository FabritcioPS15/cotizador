...
function PdfTemplate({ quote, client, room, dimensions, material, notes, calc }: { quote: { number: string; date: string; valid: string }; client: { name: string; phone: string; email: string }; room: { environment: string; furniture: string; quantity: string }; dimensions: { width: string; height: string; depth: string }; material: { brand: string; collection: string; color: string; thickness: string; edge: string }; notes: string; calc: { beforeTax: number; igv: number; total: number; qty: number } }) {
  const description = `${room.furniture} de ${room.environment.toLowerCase()} en melamina ${material.thickness} color ${material.color}. Incluye herrajes estándar, acabado ${material.collection} y tapacanto ${material.edge}. Medidas: ${dimensions.width} × ${dimensions.height} × ${dimensions.depth} cm.`
  return <div id="pdf-template" aria-hidden="true">
    <div className="pdf-header"><div className="pdf-company-wrap"><div className="pdf-brand"><span className="pdf-brand-mark">M</span><span>MODIRU <small>MUEBLES</small></span></div><div className="pdf-company">MODIRU MUEBLES S.A.C. · RUC: 20601234567<br />Av. Principal 123, Lima · +51 987 654 321 · www.modiru.pe<br />Diseño, fabricación e instalación de mobiliario a medida</div></div><div className="pdf-quote"><span className="pdf-quote-label">COTIZACIÓN</span><strong>{quote.number}</strong><span>Fecha de emisión: {quote.date}</span><span>Vigencia: {quote.valid}</span></div></div>
    <div className="pdf-title-row"><h1>Propuesta económica</h1><span>Documento comercial</span></div>
    <div className="pdf-block"><div className="pdf-block-title">INFORMACIÓN DEL CLIENTE Y PROYECTO</div><div className="pdf-info-grid"><div><p><b>Cliente:</b> {client.name}</p><p><b>DNI/RUC:</b> Por confirmar</p><p><b>Correo:</b> {client.email}</p><p><b>Teléfono:</b> {client.phone}</p></div><div><p><b>Ambiente:</b> {room.environment}</p><p><b>Tipo de mueble:</b> {room.furniture}</p><p><b>Medidas:</b> {dimensions.width} × {dimensions.height} × {dimensions.depth} cm</p><p><b>Entrega estimada:</b> 15 días hábiles · <b>Asesor:</b> Equipo MODIRU</p></div></div></div>
    <table className="pdf-table"><thead><tr><th>CONCEPTO / DESCRIPCIÓN</th><th>CANT.</th><th>PRECIO UNITARIO</th><th>TOTAL</th></tr></thead><tbody><tr><td><strong>{room.furniture} · {room.environment}</strong><br /><span>{description}</span></td><td>{calc.qty}</td><td>{money(calc.beforeTax / calc.qty)}</td><td><strong>{money(calc.beforeTax)}</strong></td></tr></tbody></table>
    <div className="pdf-footer-grid"><div><div className="pdf-block pdf-small-block"><div className="pdf-block-title">OBSERVACIONES</div><p>{notes || 'Sin observaciones.'}</p><p>Incluye fabricación, acabados, herrajes estándar e instalación. No incluye trabajos de gasfitería, electricidad ni modificaciones estructurales.</p></div><div className="pdf-block pdf-small-block"><div className="pdf-block-title">CONDICIONES COMERCIALES</div><p><b>Forma de pago:</b> 50% de adelanto · 50% contra entrega.</p><p><b>Garantía:</b> 12 meses por defectos de fabricación.</p><p><b>Validez:</b> {quote.valid} desde la fecha de emisión.</p></div></div><div className="pdf-block pdf-small-block"><div className="pdf-block-title">AUTORIZACIÓN</div><div className="pdf-signature" /><p>Firma y aceptación del cliente</p><p><b>Asesor:</b> Equipo MODIRU</p><p><b>Generado:</b> {quote.date}</p></div><div className="pdf-totals"><div><span>Subtotal</span><b>{money(calc.beforeTax)}</b></div><div><span>IGV (18%)</span><b>{money(calc.igv)}</b></div><div className="pdf-grand-total"><span>VALOR FINAL</span><b>{money(calc.total)}</b></div><small>Precios expresados en soles peruanos</small></div></div>
    <div className="pdf-validity">Gracias por confiar en MODIRU · Cotización válida por {quote.valid}</div>
  </div>
}
